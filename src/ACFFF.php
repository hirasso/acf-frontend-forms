...
class ACFFF
{
    ...
    protected string $prefix = 'acfff';

    /**
     * During acfff()->form(), this contains the current $form
     */
    public ?Form $current_form = null;
    ...
    public function add_hooks(): void
    {
        ...
        \add_filter('acf/validate_value/type=text', [$this, 'fix_max_length_validation'], 20, 4);
        \add_filter('acf/validate_value/type=textarea', [$this, 'fix_max_length_validation'], 20, 4);
    ...
    public function frontend_assets(): void
    {
        ...
        \wp_enqueue_script('acfff', $this->asset_uri('assets/acfff.js'), ['jquery', 'acf-input'], null, false);

        // enqueue plugin styles
        \wp_enqueue_style('acfff', $this->asset_uri('assets/acfff.css'), [], null, 'all');
    ...
    {
        \wp_enqueue_style("acfff-admin", $this->asset_uri('assets/acfff-admin.css'));
    }
    ...
    public function render_upload_instructions(array $field): array
    {
        ...
        if ($max_size) {
            $settings->restrictions['max_size'] = [
                'value' => $max_size,
                'error' => \sprintf(\__('File size must not exceed %s.', 'acf'), \acf_format_filesize($max_size)),
            ];
            $hints[] = \__('Max', 'acf') . ' ' . \acf_format_filesize($max_size);
        }

        \ob_start(); ?>

        <div class="acfff-instructions" hidden data-settings='<?= \json_encode($settings) ?>'>
            <div class="acfff-instructions__title"><?= $field['label'] ?></div>
            <div class="acfff-instructions__body"><?= \implode(', ', $hints) ?></div>
        </div>
     ...
     */
    protected function glue_last_two(array $array): array
    {
    ...
    {
        $success = \apply_filters("hirasso/acfff/submit_form/success", true, $post_id);
        $success = \apply_filters("hirasso/acfff/submit_form/success/id={$form['id']}", true, $post_id);
    ...
    public function validate_value(string|bool $valid, mixed $value, array $field): string|bool
    {
        ...
        if ($field['required'] && empty($value)) {
            $valid = match($field['type']) {
                default => \__('Please fill out this field', 'acfff'),
                'radio', 'select' => \__('Please select an option', 'acfff'),
                'image' => \__('Please add an image', 'acfff'),
                'file' => \__('Please add a file', 'acfff')
            };
        }
     ...
     * @param array<string, mixed> $field
     * @see https://support.advancedcustomfields.com/forums/topic/escaping-in-text-fields/
     */
    public function fix_max_length_validation(string|true $valid, ?string $value, array $field): string|true
    {
