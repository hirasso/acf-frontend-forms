...
export function isAtRootDir() {
  return existsSync(resolve(cwd(), "package.json")) && existsSync(resolve(cwd(), "composer.json"));
}
...
export function getInfosFromComposerJSON() {
  ...
  if (!fullName.includes("/")) {
    throw new Error(`Invalid name found in composer.json. It must be 'vendor-name/package-name'`);
  }
...
export function createRelease() {
  ...
  run(`composer dump-autoload --working-dir=${scopedFolder} --classmap-authoritative`);
...
function readFile(path) {
  return existsSync(path) ? readFileSync(path, { encoding: "utf-8" }) : undefined;
}
...
export async function pushReleaseToDist() {
  ...
  if (hasValidDirectories !== true) {
    error(`The validation of the scoped and dist folder failed.`, `Did you run 'release:prepare'?`);
  }
...
export async function patchVersion() {
  ...
  writeFileSync(fileName, contents.replace(versionRegexp, `* Version: ${packageVersion}`), "utf8");
